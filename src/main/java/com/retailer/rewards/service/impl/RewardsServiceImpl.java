...
public class RewardsServiceImpl implements RewardsService {
	...
	@Override
	public void calculateRewardPoints(List<Rewards> rewardsList) {
		for (Rewards rewards : rewardsList) {
			Integer rewardpoints = calculateRewardPoints(rewards);
			rewards.setPoints(rewardpoints);
			repositoy.save(rewards);
		}
	}

	private Integer calculateRewardPoints(Rewards rewards) {
		Integer points = 0;
